...
try {
    ...
    $connection.ConnectionString = "Server=$serverName;Database=$databaseName;Integrated Security=True;TrustServerCertificate=True;"
    $connection.Open()
    
    $cmd1 = $connection.CreateCommand()
    $cmd1.CommandText = $dropSql
    ...
    Write-Host "✅ Amazon-style diverse search deployed!" -ForegroundColor Green
    $connection.Close()
}
catch {
